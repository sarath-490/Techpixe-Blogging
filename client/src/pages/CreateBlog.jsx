...
export default function CreateBlog() {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            setImage(data); // Returns path like /uploads/filename.jpg
            setUploading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await api.post('/posts', {
                ...
                content,
                excerpt,
                featuredImage: image || 'https://images.unsplash.com/photo-1677442136019-21780efadfee?auto=format&fit=crop&q=80&w=1000',
                author: authorName || user?.username || 'Admin' // Use dynamic author from input
            });
            if (data.success) {
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <Helmet><title>Create Blog - Admin</title></Helmet>

            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center">
                    <button onClick={() => navigate('/admin')} className="mr-4 p-2 hover:bg-slate-800 rounded-full transition-colors">
                        <ArrowLeft className="w-5 h-5 text-slate-400" />
                    </button>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Create New Blog</h1>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800 p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="block w-full border border-gray-300 dark:border-slate-700 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white dark:bg-slate-800 text-slate-900 dark:text-white transition-shadow" placeholder="e.g. The Future of AI Agents" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Author Name</label>
                        <input type="text" value={authorName} onChange={(e) => setAuthorName(e.target.value)} className="block w-full border border-gray-300 dark:border-slate-700 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white dark:bg-slate-800 text-slate-900 dark:text-white transition-shadow" placeholder=" e.g. John Doe" />
                    </div>
                    ...
                    {image && (
                        <div className="mt-4 relative w-full h-48 bg-slate-800 rounded-lg overflow-hidden border border-slate-700">
                            <img src={image}
                                ...
                                onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1677442136019-21780efadfee?auto=format&fit=crop&q=80&w=1000' }}
                            />
                        </div>
                    )}
                </div>
                ...
                <div>
                    ...
                    <textarea rows="3" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} className="block w-full border border-gray-300 dark:border-slate-700 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white dark:bg-slate-800 text-slate-900 dark:text-white transition-shadow" placeholder="Short description for home page..." />
                </div>
                ...
                <div className="flex justify-end pt-6 border-t border-gray-200 dark:border-slate-700">
                    ...
                    <button type="submit" disabled={submitting} className="flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 disabled:opacity-50">
                        {submitting ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Plus className="w-4 h-4 mr-2" />} Publish Blog
                    </button>
                </div>
            </form>
        </div>
